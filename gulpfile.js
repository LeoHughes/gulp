...
gulp.task('jade',function(){
    return gulp.src(['views/**/*.jade','views/*.jade','!views/layout/*jade'])
...
gulp.task('watch-jade',['jade'],function(){
    return  gulp.watch(['views/**/*.jade','views/*.jade'],['jade']);
});
...
gulp.task('lessToCss', function () {
    return gulp.src('src/less/*.less') //任务文件
               .pipe(less()) //编译
               .pipe(cleanCSS()) //压缩css
...
//监听less文件
gulp.task('watch-less',['lessToCss'],function() {
    return gulp.watch('src/less/**/*.less', ['lessToCss']);    //设定监听文件和执行任务
});
...
gulp.task('watch-js',['scripts'],function(){
   return gulp.watch(['src/js/*.js','src/js/**/*.js'],['scripts']);
});
...
//本地服务器
gulp.task('webserver',function(){   //启动服务器先执行编译less、压缩js和编译jade
